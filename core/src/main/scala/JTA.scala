...
/**
 * Transaction interceptor that listens to EJB 3's <code>javax.ejb.TransactionAttribute</code> annotation.
 * Implements EJB 3's transaction semantics (REQUIRED, REQUIRES_NEW, MANDATORY, SUPPORTS, NEVER, NOT_SUPPORTED)
 */
trait EjbTransactionInterceptor extends Interceptor with TransactionProtocol {
  ...
  abstract override def invoke(invocation: Invocation): AnyRef = if (matches(matchingJTAAnnotation, invocation)) {
    ...
    log.debug("Starting JTA transaction [%s] @ %s :: %s", txType, invocation.target.getClass, invocation.method)
 ...
 * Transaction service.
 */
object TransactionService extends TransactionProtocol {
 ...
 */
class SkalmanTransactionManagerLookup extends org.hibernate.transaction.TransactionManagerLookup {
  def getTransactionManager(props: java.util.Properties): TransactionManager = TransactionService.getTransactionManager
...
class EntityManagerSynchronization(
  ...
  extends Synchronization with Logging {
  ...
  def afterCompletion(status: Int) = {
    ...
    if (status == Status.STATUS_ROLLEDBACK ||
        status == Status.STATUS_ROLLING_BACK ||
        status == Status.STATUS_MARKED_ROLLBACK) {
        ManagedComponentContext.closeEntityManager // destroy EntityManager on rollback
     }
  }
}
...
trait TransactionProtocol extends Logging {
  ...
  def withTxRequired[T](body: => T): T = {
    val tm = ManagedComponentContext.getTransactionManager
    if (!isExistingTransaction(tm)) {
      ManagedComponentContext.getEntityManager
      tm.begin
      joinTransaction
      try {
        body
  ...
  def withTxMandatory[T](body: => T): T = {
    if (!isExistingTransaction(ManagedComponentContext.getTransactionManager)) throw new TransactionRequiredException("No active TX at method with TX type set to MANDATORY")
    body
  }
  ...
  def withTxNever[T](body: => T): T = {
    if (isExistingTransaction(ManagedComponentContext.getTransactionManager)) throw new SystemException("Detected active TX at method with TX type set to NEVER")
    body
  }
  ...
  protected def handleException(tm: TransactionManager, e: Exception) = {
    if (isExistingTransaction(tm)) {
      ...
      if (!e.isInstanceOf[NoResultException] &&
        ...
        log.debug("Setting TX to ROLLBACK_ONLY, due to: %s", e.toString)
        tm.setRollbackOnly
      }
    }
    throw e
  ...
  /**
   * Returns the transaction attribute type for a specific class or method.
   *
   * @param klass
   * @param method
   * @return the TX type
   */
  protected def getTransactionAttributeTypeFor(klass: Class[_], method: Method): TransactionAttributeType = {
    // check the class
    var tx: TransactionAttribute = method.getAnnotation(classOf[TransactionAttribute]).asInstanceOf[TransactionAttribute]
    // check the method
    if (tx == null) tx = klass.getAnnotation(classOf[TransactionAttribute]).asInstanceOf[TransactionAttribute]
    // grab the TX type
    if (tx != null) tx.value
    else REQUIRED // default for CMT components - EJB 3 Spec/Chapter 9.4
  }

  private def joinTransaction = {
    val em = ManagedComponentContext.getEntityManager
    val tm = ManagedComponentContext.getTransactionManager
